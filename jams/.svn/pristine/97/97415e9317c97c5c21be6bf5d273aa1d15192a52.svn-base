...
public class FloatAccessor implements DataAccessor {
    ...
    Attribute.Float componentObject;

    Attribute.Float[] entityObject;
    ...
    public FloatAccessor(DataFactory dataFactory,Attribute.Entity[] entities, JAMSData dataObject, String attributeName, int accessType) throws JAMSEntity.NoSuchAttributeException {
        ...
        for (int i = 0; i < entities.length; i++) {
            if (entities[i].existsAttribute(attributeName)) {
                try {
                    entityObject[i] = (JAMSFloat) entities[i].getObject(attributeName);
                } catch (JAMSEntity.NoSuchAttributeException nsae) {
                }
            } else {
                if (accessType != DataAccessor.READ_ACCESS) {
                    entityObject[i] = dataFactory.createFloat();
    ...
    public void initEntityData() {
        for (Attribute.Float v : entityObject) {
