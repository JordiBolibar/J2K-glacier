	...
	public Iterable<String> getImagelinkTitles(String image)
        ...
        public boolean removeArticle(String title) 
            ...
            try {
                ...
                System.out.println("could not decode url, because:" + e.toString());
                return false;
            }
          
            System.out.println("Removing Article:" + title);
            Hashtable<String, String> tab = new Hashtable<String, String>();
            PostDelete DeleteAction = new PostDelete(title,tab,true);
            try {
                //performAction(new GetEnvironmentVars(title, tab, login));                                        
                //two Actions are needed: 
                //(1) deleting request
                //(2) confirmation
                //switch is controlled by bFirstCall flag
                performAction(DeleteAction);
                PostDelete DeleteAction2 = new PostDelete(title,tab,false);
                performAction(DeleteAction2);
            }catch(Exception e) {
                System.out.println("Could not delete content, because: " + e.toString());
                return false;
            }
