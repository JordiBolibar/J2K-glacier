 ...
 */
public class PatchedSpiderWebPlot extends Plot implements Cloneable, Serializable {
	/**
	 * For serialization.
	 */
	...
	/**
	 * Number of ticks should be >= 1
	 */
	private int numberOfTicks = 1;
	/**
	 * Determines whether that data should be interpreted is inverted data. I.E.
	 * each data points real value is -value. This will allow tick labels to be
	 * changed.
	 */
	private boolean dataIsInverted = false;

	/**
	 * Creates a default plot with no dataset.
	 */
	public PatchedSpiderWebPlot() {
		this(null);
	}
	 ...
	 */
	public PatchedSpiderWebPlot(CategoryDataset dataset) {
	 ...
	 */
	public PatchedSpiderWebPlot(CategoryDataset dataset, TableOrder extract) {
		super();
		if(extract == null) {
	...
	/**
	 * Sets the head outline stroke for all series. When
	 * <code>null</code> {@link #getSeriesOutlineStroke(int)} is used. Sends a {@link
...
//		double[] ticks = {0.5d, 1d};
		for(int i = 0; i < numberOfTicks; i++) {
			double tick = ((double) (i + 1)) / numberOfTicks;
	...
	public void setDataInverted(boolean inverted) {
		dataIsInverted = inverted;
	}
...
// g2.drawRect((int) labelX, (int) labelY - labelH, labelW, labelH); // test rectangle
	 ...
	 * category. This allows us to plot
	 * <code>BY_ROW</code> or
	...
	public boolean equals(Object obj) {
		...
		if(!(obj instanceof PatchedSpiderWebPlot)) {
			return false;
		}
		if(!super.equals(obj)) {
			return false;
		}
		PatchedSpiderWebPlot that = (PatchedSpiderWebPlot) obj;
	...
	public Object clone() throws CloneNotSupportedException {
		PatchedSpiderWebPlot clone = (PatchedSpiderWebPlot) super.clone();
