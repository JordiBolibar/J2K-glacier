...
public class PhosphorousFlowReader extends JAMSComponent {
	...
	for (int i=0;i<N+1;i++) {
	    ...
	    for (int j=0;j<N;j++) {
	        M.element[j][0] = field_fd[j].getDouble("inputP_sum");
	    ...
	    e.setDouble("ist_input",field_fd[N].getDouble("inputP_sum"));
	    e.setDouble("min_input",field_fd[0].getDouble("inputP_sum"));
        }
