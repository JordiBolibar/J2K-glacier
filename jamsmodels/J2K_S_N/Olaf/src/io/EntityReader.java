...
@Keywords
    ("I/O")
@SourceInfo
    ("$HeadURL: http://svn.javaforge.com/svn/oms/branches/oms3.prj.ceap/src/io/EntityReader.java $")
@VersionInfo
    ("$Id: EntityReader.java 963 2010-02-11 20:38:23Z odavid $")
@License
...
public class EntityReader {
    ...
    private void readReaches() throws IOException {
        ...
        for (String[] row : table.rows()) {
            ...
            reach.deepsink = Double.parseDouble(row[7]);
            reaches.add(reach);
            System.out.print(".");
            if (log.isLoggable(Level.INFO)) {
                log.info(Arrays.toString(row));
    ...
    private void readLanduse() throws IOException {
        ...
        for (String[] row : table.rows()) {
            ...
            lu.albedo = Double.parseDouble(row[2]);
    ...
    private void readGW() throws IOException {
        ...
        for (String[] row : table.rows()) {
            ...
            geo.Kf_geo = Double.parseDouble(row[7]);
