...
public class J2KProcessGroundwater_D_N_1_test extends JAMSComponent {
    ...
    @JAMSVarDescription(access = JAMSVarDescription.AccessType.READ,
    update = JAMSVarDescription.UpdateType.INIT,
    description = "RG1 lateral correction factor")
    public JAMSDouble gwRG1LatFact;
    
    @JAMSVarDescription(access = JAMSVarDescription.AccessType.READ,
    update = JAMSVarDescription.UpdateType.INIT,
    description = "RG1 vertical correction factor")
    public JAMSDouble gwRG1VertFact;
    ...
    @JAMSVarDescription(
        ...
        @JAMSVarDescription(
            ...
            public JAMSDouble gwLatVertRG1;
    ...
    double run_maxRG1, run_maxGW, 
           run_actRG1, run_actGW, 
           run_inRG1,  run_inGW, 
           run_outRG1, run_outGW, 
           run_genRG1, run_genGW,
           run_potLatRG1, run_potVertRG1,
           run_satGW,
           run_pot_RG1, run_pot_GW, 
           run_pre_outGW, run_pre_actGW, 
           run_k_RG1, run_k_GW,
           run_RG1_rec, 
           run_RG1lat_rec, run_RG1vert_rec, 
           run_GW_rec, 
    ...
    public void run() throws JAMSEntity.NoSuchAttributeException {
        ...
        this.entity = entities.getCurrent();
        ...
        this.run_kSink = kSink.getValue();                  //[cm/d]
        ...
        // Berechnung des Recession-Coeffizient
        this.run_RG1lat_rec = this.run_Kf_geo * this.gwRG1LatFact.getValue() / 1000 / 8640;  //[m/d] --> [l/d]
    ...
    public boolean calcRG1Transfer(){

        double RG1Thickness = this.run_upperBorder - this.run_gwTableUpper;

        //Test: Grundwasserspiegellage im Boden?
        if (RG1Thickness < 0){
            //ja? dann keine Transferzone vorhanden, RG1=0
            this.run_actRG1 = 0;
            this.run_outRG1 = 0;
            this.run_genRG1 = 0;
        }else{
             //Berechnung der maximal möglichen Speicherinhalt der Transferzone (zwischen Boden und Grundwasserspiegellage)
            this.run_maxRG1 = RG1Thickness * run_area * run_Peff * 1000;  //[l]
            
            //Übergabe des Perkolatioswassers an den RG1
            this.run_actRG1 = this.run_actRG1 + this.run_percolation;
            
            // ACHTUNG! Wenn RG1 voll, dann ist er eigentlich nicht existent, weil er komplett zur gesättigten Zone zugerechnet werden müsste!
            ...
            // Lateraler Ausflus durch Kalibrierungsfaktor gwLatVertRG1 möglich, 0 perkolationswasser nur vertikal, >0 lateraler Fluss möglich
            double slope_weight = Math.tan(this.run_slope * (Math.PI / 180.));
            double gradh = ((1 - slope_weight) * 1 * this.gwLatVertRG1.getValue());
            ...
            this.run_potLatRG1 = ((1 - gradh) * this.run_actRG1);
            this.run_potVertRG1 = (gradh * this.run_actRG1);

            // Vertikaler Ausfluss
            // Perkolationswiderstand durch Kf_geo, Fließlänge (Dicke der Transferzone) und Kalibrierfaktor: gwRG1VertFact
            // Q = k * A * grad
            run_pot_GW = this.run_Kf_geo * this.run_area * this.gwRG1VertFact.getValue() * 1000 * RG1Thickness;

            if (run_pot_GW > this.run_potVertRG1){
                run_pot_GW = run_potVertRG1;
            }

            this.run_actRG1 = this.run_actRG1 - this.run_pot_GW;
            ...
            // Lateraler Ausfluss aus RG1
            // Rückhaltewiderstand für Lateralen Ausfluss: gwRG1LatFact = run_RG1lat_rec

            double k_rg1 = 1 / this.run_RG1lat_rec;
            if (k_rg1 > 1) {
                k_rg1 = 1;
            }
            ...
            double rg1_out = this.run_Kf_geo * this.run_slope * this.run_RG1lat_rec * RG1Thickness * this.run_gwFlowWidth * 1000;

            if (rg1_out > this.run_potLatRG1){
                rg1_out = run_potLatRG1;
            }
    ...
    public boolean calcDeepSink(){
        ...
        this.run_deepSinkW = this.run_area * this.run_kSink * this.run_gwTableUpper * this.run_deepSinkFactor * 10; //[l/d]
    ...
    private boolean calcDarcyGWout() throws JAMSEntity.NoSuchAttributeException {
        ...
        // Ausfluss aus RG1 alte Variante
...
*/
        ...
        GW_out = GW_out_m3 * 1000 * 8640;     //[l/d]
