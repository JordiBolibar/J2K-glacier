/*
 * J2KProcessSnow_bidaily_global_season.java
 ...
 * 6. the 'seasonal' model includes a modulation of the t_factor with seasonal cycle (max values on June, 21). 
 * The modulation is done by adding a sinus with amplitude Amp (=additional model parameter).
...
// IG ++: adds by IG for a global snow model
    
    @JAMSVarDescription(
            access = JAMSVarDescription.AccessType.READ,
            update = JAMSVarDescription.UpdateType.RUN,
            description = "aspect influence on t_factor"
            )
            public JAMSDouble a_asp;

    @JAMSVarDescription(
            access = JAMSVarDescription.AccessType.READ,
            update = JAMSVarDescription.UpdateType.RUN,
            description = "altitude influence on t_factor"
            )
            public JAMSDouble a_alt;

    @JAMSVarDescription(
            access = JAMSVarDescription.AccessType.READ,
            update = JAMSVarDescription.UpdateType.RUN,
            description = "latitude influence on t_factor"
            )
            public JAMSDouble a_lat;

    @JAMSVarDescription(
	    access = JAMSVarDescription.AccessType.READ,
            description = "HRU elevation")
    	    public Attribute.Double entityElevation;

    @JAMSVarDescription(access = JAMSVarDescription.AccessType.READ,
            description = "HRU aspect")
            public Attribute.Double entityAspect;

    @JAMSVarDescription(access = JAMSVarDescription.AccessType.READ,
            description = "HRU latitude")
    	    public Attribute.Double entityLatitude;
...
// IG --
    ...
    @JAMSVarDescription(
            access = JAMSVarDescription.AccessType.READ,
            update = JAMSVarDescription.UpdateType.INIT,
            description = "t_factor seasonal Amplitude",
            lowerBound = -10.0,
            upperBound = 10.0,
            defaultValue="0",
            unit = "°C"
            )
            public JAMSDouble Amplitude;
    ...
    public void run() throws JAMSEntity.NoSuchAttributeException {
		...
		double aa_asp=a_asp.getValue();
		double aa_alt=a_alt.getValue();	        
		double aa_lat=a_lat.getValue();
		double full_temp_fac= temp_fac+aa_asp*Math.sin(rad(entityAspect.getValue())-3.14/2.) + aa_alt*(entityElevation.getValue()-1000.)/1000.;
			// lat supprimée à l'échelle d'un BV : + aa_lat*(entityLatitude.getValue()-6460000)/1000000.;
	        
        int dayCount = time.get(Attribute.Calendar.DAY_OF_YEAR) - 1;

                temp_fac= Amp * Math.sin (2 * 3.14 * (dayCount - 142)/ 365) + temp_fac;
	        this.run_snowMelt = 0; 
...
// we want to have the snow accumulation at each timestep
	        ...
	        double in_meltTempmax = ( in_maxTemp + in_meanTemp)/2;
	        double in_meltTempmin = ( in_minTemp + in_meanTemp)/2;
	        
	        if((in_meltTempmax >= TRS) && (this.run_snowDepth > 0)){
	            this.calcMetamorphosis(in_meltTempmax, TRS, temp_fac, rain_fac, ground_fac, run_area, SAC, critDens);
	        }
	        if((in_meltTempmin >= TRS) && (this.run_snowDepth > 0)){
	            this.calcMetamorphosis(in_meltTempmin, TRS, temp_fac, rain_fac, ground_fac, run_area, SAC, critDens);
	        }
 ...
 // Francois Tilmant : we force new snow density = 0.3 to avoid enormous snowDepth 
    ...
    private boolean calcMetamorphosis(double temp, double TRS, double temp_fac, double rain_fac, double ground_fac, double area, double SAC, double critDens){
        ...
        double potMR = 0;
        potMR = this.calcPotMR_semiComp(temp, TRS, temp_fac, rain_fac, ground_fac, area);
        
        if(Math.abs(this.run_coldContent) >= potMR){
            this.run_coldContent = this.run_coldContent + potMR;
            potMR = 0;
        } else{
            potMR = potMR + this.run_coldContent;
            this.run_coldContent = 0;
        }
        
        potMR = (potMR * area)/2; //MT formulation bi-journalière
        ...
        double deltaSnowDepth = potMR / (this.run_dryDens * area);
        ...
        this.run_drySWE = this.run_drySWE - potMR;
        potMR = 0;
        ...
        this.calcRainSnowSettlement(this.in_rain + potMR);
        this.in_rain = 0;
    ...
    private double calcPotMR_semiComp(double temp, double TRS, double temp_fac, double rain_fac, double ground_fac, double area){
        ...
        double potMR = (temp_fac * (meltTemp - TRS) + ground_fac + rain_fac * (this.in_rain / area) * meltTemp);
