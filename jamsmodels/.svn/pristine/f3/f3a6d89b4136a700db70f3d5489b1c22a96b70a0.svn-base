/*
 * J2KProcessLumpedSoilWater_distInfiltCoeff_test.java
 * Created on 28. June 2014, 11:59
 *
...
/**
 *
 * @author Peter Krause+Meriem Labbas+Francois Tilmant
 */
...
extends JAMSComponent {
    ...
    @JAMSVarDescription(
            access = JAMSVarDescription.AccessType.READ,
            description = "infiltration coefficient",
            unit="",
            lowerBound = 0.0,
            upperBound = 1.0
            )
            public Attribute.Double InfiltrationCoeff;
    ...
    @JAMSVarDescription(
        ...
        @JAMSVarDescription(
            access = JAMSVarDescription.AccessType.WRITE,
            description = "statevar surface Runoff from excess saturation",
            unit="L"
            )
            public Attribute.Double satRD1;
    ...
    public void run() {
        ...
        run_overlandflow1 = this.run_overlandflow;
        ...
        directRD1.setValue(this.run_overlandflow1);
        infRD1.setValue(this.run_deltaInf);
        satRD1.setValue(this.run_satRD1);
    }
    ...
    private void calcInfImperv(){
        this.run_overlandflow = this.run_overlandflow + (1 - InfiltrationCoeff.getValue()) * this.run_infiltration;
        this.run_infiltration = this.run_infiltration * InfiltrationCoeff.getValue();
    }
    ...
    private double calcLPSInflow(double infiltration){
        ...
        if(this.run_actLPS > this.run_maxLPS){
            ...
            this.run_satRD1 = this.run_actDPS ;
